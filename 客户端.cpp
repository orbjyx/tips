...
int net(btime group[20]) {
		...
		if (group[j].count != 0) {
			for (k = 0; k < group[j].count; k++) {
				if (group[j].message[k].quxiao == 0) {
					i = j + 1;
					outtextxy(x, y, _T("姓名："));
					strcpy(buff, group[j].message[k].name);
					outtextxy(x + 60, y, buff);
					r = send(clientSocket, buff, strlen(buff), NULL);

					y = y + 30;
					outtextxy(x, y, _T("性别："));
					strcpy(buff, group[j].message[k].sex);
					outtextxy(x + 60, y, buff);
					r = send(clientSocket, buff, strlen(buff), NULL);

					y = y + 30;
					outtextxy(x, y, _T("联系方式："));
					strcpy(buff, group[j].message[k].phone);
	...
	setlinecolor(YELLOW);
	rectangle(290, 480, 340, 520);
	outtextxy(295, 485, _T("返回"));
	MOUSEMSG m;
	while (true)
	{
		m = GetMouseMsg();
		if (m.uMsg == WM_LBUTTONDOWN) {
			if ((m.x >= 290 && m.x <= 340) && (m.y >= 480 && m.y <= 520))
			{
				quit(group);
			}
			break;
